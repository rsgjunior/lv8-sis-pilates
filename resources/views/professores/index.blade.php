        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    @if ($pesquisa)
                        <a href="{{ route('professores.index') }}" class="btn btn-sm btn-danger">
                    ...
                    <table class="table table-hover" id="tabela">
                        ...
                        <tbody>
                            @foreach ($professores as $professor)
                                <tr>
                                    <td>
                                        <img class="table-img" @if ($professor->foto) src="{{ url('/storage/fotos/professores/' . $professor->id . '/' . $professor->foto) }}" 
                    @else
                          src="{{ url('/img/default.jpg') }}" @endif>
                                    </td>
                                    <td><a
                                            href="{{ route('professores.show', ['professor' => $professor->id]) }}">{{ $professor->nome }}</a>
                                    </td>
                                    <td>{{ count($professor->turmas) }}</td>
                                    <td>{{ $professor->email }}</td>
                                    <td>{{ $professor->idade }} anos</td>
                                    <td>
                                        <a href="{{ route('professores.show', ['professor' => $professor->id]) }}"
                                            class="d-inline-block btn btn-primary btn-sm">
                                            <i class="fa fa-eye" aria-hidden="true"></i>
                                            Ver
                                        </a>

                                        <a href="{{ route('professores.edit', ['professor' => $professor->id]) }}"
                                            class="d-inline-block btn btn-info btn-sm">
                                            <i class="fa fa-edit" aria-hidden="true"></i>
                                            Editar
                                        </a>
                                        <form class="d-inline-block"
                                            action="{{ route('professores.destroy', ['professor' => $professor->id]) }}"
                                            method="post">
                                            @csrf
                                            @method('DELETE')
